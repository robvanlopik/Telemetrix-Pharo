...
The drivers can also be used with Pots (github://robvanlopik/Pots) using subclasses of `PotsTMXDriver`.
## Usage
...
```
After that you have to assign functions to pins. This is mostly done by a method called `beXyzOnPin:`.
Digital and anlog output work as expected. Inputs present the latest value. `beAnalogInputOnPin:differential:` is useful to restrict the number of announcements generated by noise on the AD-converter.
...
Metacello new
	...
	load
```
The Telemetrix server on the microprocessor can be installed through the Arduino IDE. Look for files->examples->Telemetrix. In the case of Wifi you have to edit the .ino file for the Wifi SSID and password. In some cases you may need to comment out a line about LED_BUILTIN_SUPPORTED
## Notes
